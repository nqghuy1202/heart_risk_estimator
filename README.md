# Heart Disease Prediction — AdaBoost, Django API, React

Predicting whether a patient has heart disease from thirteen routine clinical measurements. An AdaBoost
...
the 106 who do not. That asymmetry is the one worth having here: a missed case sends someone home
untreated, while a false alarm costs a follow-up test. Recall on the disease class is therefore the metric
this model should be judged on, not accuracy — and at 92.9% it is the strongest of the four figures above.
...
collected in 1988. The copy used here (`Demo_Adaboost/heart.csv`) is the widely redistributed 1,025-row
...
The 1,025 rows are an expanded version of the original 303-record UCI set and contain duplicated records
— see [Limitations](#limitations).

## Architecture

```
heart_disease.ipynb  ──dump()──▶  adaboost.joblib  ──load()──▶  basicConcepts/views.py
                                                                       │
              basicConcepts/form.py  (TestForm: 13 validated fields) ──┤
                                                                       │
                                        GET  /api/schema/   ◀──────────┤
                                        POST /api/predict/  ◀──────────┘
                                                 ▲
                                        ...
                                        frontend/src  (React + TypeScript)
                                                 │  vite build
                                                 ▼
                             basicConcepts/static/frontend/{app.js,app.css}
                                                 │  {% static %}
                                                 ▼
...
**The form is the validation layer, and it stayed that way.** `views.py` never reads the request body
directly — the JSON is bound to the same `TestForm` the server-rendered version used, and only
`form.cleaned_data` reaches the classifier. Each field carries the range the dataset documentation
defines, and `TypedChoiceField(coerce=int)` means the view receives real integers rather than strings.

**The client is told what the form is, not born knowing.** `GET /api/schema/` returns every field's
label, help text, bounds and options, plus the section grouping, all derived from `form.py`. The React
app renders from that and never hard-codes a field name, a range or a choice — so the browser cannot
drift from what the server will accept, and regrouping the UI is still a one-line change in Python.
...
React 19 · TypeScript 5.9 · Vite 7 · Vitest + React Testing Library (14 tests covering the form, the
...
The one thing lost in moving to React is the no-JavaScript path: the previous version was a plain form
post and worked with scripting disabled. The API is still usable directly (see above), and the page says
so if JavaScript is off.

## Quick start

```bash
git clone https://github.com/nqghuy1202/heartdisease_adaboost.git
cd heartdisease_adaboost/Demo_Adaboost
...
under `Demo_Adaboost/basicConcepts/static/frontend/`.
...
plain English beneath its field — `oldpeak` is not a term most people filling in this form would know.
Out-of-range values are rejected server-side with the accepted range stated in the message, and the
other twelve answers are preserved.

## Project structure

```
heartdisease_adaboost/
├── Demo_Adaboost/
...
│   │   ├── components/            # TopBar, PredictionForm, Field, Verdict
│   │   └── styles/app.css         # the Hairline Editorial system
...
Stated plainly, because they affect how much weight the numbers above deserve.

- **`n_estimators` was selected on the test set.** There is no validation split and no cross-validation,
  so the reported 87.8% is mildly optimistic — the test set influenced a modelling choice. The honest fix
  is a three-way split or `GridSearchCV` on the training set alone.
...
- Replace test-set selection with `GridSearchCV` on the training split, and deduplicate before splitting
  — both metrics and honesty improve.
